"""Vercel Python serverless function for the Polemicyst transcript service.

Mirrors the local stdlib service in `../server.py`: given a YouTube URL it
returns the transcript via `youtube-transcript-api`.
...
Routing: `vercel.json` rewrites `/transcript` -> `/api/transcript` and
`/health` -> `/api/transcript`, so the public contract stays
`POST /transcript` + `GET /health`.

Auth: if the RENDER_SECRET env var is set, requests must send a matching
`x-render-secret` header. If unset the endpoint is open.

The KEY QUESTION this deployment answers: does `youtube-transcript-api` work
from Vercel's datacenter IP, or does YouTube block it? A 502 with an
IpBlocked/RequestBlocked/TooManyRequests error means blocked.
"""
import json
import os
import re
...
# ---- video id extraction ---------------------------------------------------
# Handles youtube.com/watch?v=, youtu.be/, /shorts/, /embed/, /live/, the `?si=`
# share param, and ids that start with an underscore (e.g. _g4l7YkDQwA).
_ID_PATTERNS = [
...
class handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):  # keep logs readable
