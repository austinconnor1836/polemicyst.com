...
export function dealHands(deck: Card[], playerCount: number): { hands: Card[][]; remaining: Card[] } {
...
function nigiriValue(type: CardType): number {
  switch (type) {
    case 'egg_nigiri': return 1
    case 'salmon_nigiri': return 2
    case 'squid_nigiri': return 3
    default: return 0
  }
}
...
export const CARD_INFO: Record<CardType, CardInfo> = {
  ...
  dumpling: { name: 'Dumpling', emoji: '🥟', color: '#60A5FA', description: '1→1, 2→3, 3→6, 4→10, 5→15' },
  ...
  salmon_nigiri: { name: 'Salmon Nigiri', emoji: '🐟', color: '#FB923C', description: '2 pts (×3 w/ wasabi)' },
  squid_nigiri: { name: 'Squid Nigiri', emoji: '🦑', color: '#A78BFA', description: '3 pts (×3 w/ wasabi)' },
  egg_nigiri: { name: 'Egg Nigiri', emoji: '🥚', color: '#FCD34D', description: '1 pt (×3 w/ wasabi)' },
  wasabi: { name: 'Wasabi', emoji: '🟢', color: '#4ADE80', description: 'Next nigiri ×3' },
  chopsticks: { name: 'Chopsticks', emoji: '🥢', color: '#94A3B8', description: 'Swap to play 2 cards' },
  pudding: { name: 'Pudding', emoji: '🍮', color: '#FBBF24', description: 'End: most +6, least −6' },
}
...
export function useChopsticks(player: Player, secondCardId: number): Player {
