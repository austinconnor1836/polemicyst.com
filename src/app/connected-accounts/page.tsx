...
export default function FeedsPage() {
  ...
  const fetchFeeds = async () => {
    ...
    try {
      ...
      console.error(err);
      setPageError('Could not load accounts. Try refreshing.');
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      console.error(err);
      setPageError('Could not load videos. Try refreshing.');
  ...
  */
  ...
  const deleteFeed = async (feedId: string) => {
    ...
    setPageError(null);
    try {
      const res = await fetch(`/api/connected-accounts/${feedId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete feed');
      await Promise.all([fetchFeeds(), fetchVideos()]);
      toast.success('Account removed');
    } catch (err) {
      console.error(err);
      setPageError('Failed to remove account. Try again.');
  ...
  const updateFeedSettings = async (feedId: string, updates: any) => {
    try {
      const res = await fetch(`/api/connected-accounts/${feedId}`, {
        method: 'PATCH',
    ...
    <div className="mx-auto w-full max-w-screen-xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6">
        <div className="relative overflow-hidden rounded-2xl border bg-gradient-to-b from-background to-background/60">
          <AccountsHeroAnimation className="absolute inset-0 opacity-90" />
