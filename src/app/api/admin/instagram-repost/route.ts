...
export async function POST(request: Request) {
  try {
    ...
    if (!IG_SOURCE_USERNAME || !IG_SOURCE_PASSWORD || !IG_TARGET_USERNAME || !IG_TARGET_PASSWORD) {
      return NextResponse.json({ error: 'Missing Instagram credentials in environment variables.' }, { status: 500 });
    }
    ...
    try {
        ...
        const sourceState = require('fs').readFileSync(process.cwd() + '/ig-state-source.json', 'utf8');
    ...
    try {
        const targetState = require('fs').readFileSync(process.cwd() + '/ig-state-target.json', 'utf8');
    ...
    const targetCollection = collections.items.find((c: any) => c.media_count > 0 && c.name.toLowerCase() === collectionName.toLowerCase());

    if (!targetCollection) {
      return NextResponse.json({ error: `Collection '${collectionName}' not found or empty on source account.` }, { status: 404 });
    }

    console.log(`Found collection: ${targetCollection.name} (ID: ${targetCollection.collection_id})`);
    ...
    try {
        ...
        console.warn("Could not get specific collection feed, falling back to recent saved items...", e);
        // Fallback: get recent saved and check manually? (Collection info isn't always on the media item itself easily)
        return NextResponse.json({ error: 'Failed to access specific collection feed.' }, { status: 500 });
    }
