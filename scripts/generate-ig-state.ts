...
async function loginAndSave(usernameKey: string, passwordKey: string, filename: string) {
  ...
  try {
    ...
    if (e.message.includes('challenge_required') || e.name === 'IgCheckpointError') {
      ...
      // Let's ask.
      const method =
        (await askQuestion('Is this SMS (0) or Authenticator App (1)? [default: 1]: ')) || '1';
  ...
  try {
    ...
    console.log(
      'Collections Found:',
      collections.items.map((c: any) => `${c.name} (ID: ${c.collection_id})`).join(', ')
    );
  } catch (err: any) {
    ...
    try {
      ...
      console.log(
        'Collections Found (API manual):',
        (body.items || []).map((c: any) => `${c.name} (ID: ${c.collection_id})`).join(', ')
      );
    } catch (err2: any) {
