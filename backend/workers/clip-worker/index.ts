...
const provocativenessEvents = new QueueEvents('score-provocativeness', { connection: redisConnection });
...
async function createClip(videoPath: string, start: number, end: number, text: string, outPath: string, aspectRatio: string) {
  ...
  const aspectRatioFilter = (() => {
    switch (aspectRatio) {
      ...
      case '9:16': default: return 'scale=720:1280,setsar=1';
    }
  })();
