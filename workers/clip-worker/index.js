...
// Queues for scoring
var provocativenessQueue = new bullmq_1.Queue('score-provocativeness', {
  connection: redisConnection,
});
var comedicQueue = new bullmq_1.Queue('score-comedic', { connection: redisConnection });
var provocativenessEvents = new bullmq_1.QueueEvents('score-provocativeness', {
  connection: redisConnection,
});
      ...
      return [
        ...
        new Promise(function (resolve, reject) {
          ...
          python.stderr.on('data', function (d) {
            return (error += d.toString());
          });
          python.on('close', function (code) {
            if (code !== 0) return reject(new Error('Transcription failed: '.concat(error)));
            try {
          ...
          return [
            ...
            new Promise(function (resolve, reject) {
              ...
              ffmpeg.on('close', function (code) {
                if (code === 0) resolve();
                else reject(new Error('FFmpeg failed ('.concat(code, ')')));
              });
            }),
          ];
      }
      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          ...
          case 1:
            ...
            return [
              4 /*yield*/,
              prisma_1.prisma.feedVideo.findUnique({ where: { id: feedVideoId } }),
            ];
          case 2:
            feedVideo = _b.sent();
          ...
          case 5:
            ...
            return [
              4 /*yield*/,
              prisma_1.prisma.feedVideo.update({
                    ...
                    return __generator(this, function (_b) {
                      switch (_b.label) {
                        case 0:
                          return [
                            4 /*yield*/,
                            provocativenessQueue.add('score', { transcript: w.text }),
                          ];
                        case 1:
                          pJob = _b.sent();
          ...
          case 7:
            ...
            topProvocative = __spreadArray([], scoredWindows, true)
              .sort(function (a, b) {
                return b.provocativeness - a.provocativeness;
              })
              .slice(0, 2);
            topComedic = __spreadArray([], scoredWindows, true)
              .sort(function (a, b) {
                return b.comedic - a.comedic;
              })
              .slice(0, 2);
            selected = new Set(
              __spreadArray(__spreadArray([], topProvocative, true), topComedic, true)
            );
          ...
          case 8:
            ...
            return [
              4 /*yield*/,
              createClip(videoPath, w.start, w.end, w.text, outPath, aspectRatio || '9:16'),
            ];
          case 9:
            _b.sent();
            console.log(
              '\u2705 Generated clip: '
                .concat(outPath, ' (P: ')
                .concat(w.provocativeness, ', C: ')
                .concat(w.comedic, ')')
            );
            // Create Video entry in DB
            return [
              4 /*yield*/,
              prisma_1.prisma.video.create({
                data: {
                  userId: userId,
